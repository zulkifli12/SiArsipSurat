...
.modal-card {
    ...
    width: 100%;
    max-width: 650px; /* Diperlebar sedikit untuk form surat yang lebih banyak field */
    padding: 36px 40px;
...
<div class="card-container">
    <div class="card-header" style="margin-bottom: 24px;">
        ...
        <div class="header-actions">
            ...
            <button type="button" class="btn btn-primary" onclick="showPremiumContactPopup('Registrasi Surat Masuk')" style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%);">
    ...
    <div class="table-responsive">
        <table id="surat-masuk-table" class="activity-table display" style="width:100%">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach($suratMasuk as $item)
                    <tr>
                        ...
                        <td>
                            <div style="font-weight: 600; color: var(--text-main);">{{ $item->pengirim }}</div>
                        </td>
                        <td>
                            <span class="badge-category">{{ $item->kategori->nama ?? 'Tanpa Kategori' }}</span>
                        </td>
                        <td>
                            <div style="font-weight: 600; color: var(--text-main); font-size: 13px;">Tgl Surat: {{ \Carbon\Carbon::parse($item->tgl_surat)->locale('id')->isoFormat('D MMM YYYY') }}</div>
                            <div style="font-weight: 500; color: var(--text-muted); font-size: 12px;">Diterima: {{ \Carbon\Carbon::parse($item->tgl_diterima)->locale('id')->isoFormat('D MMM YYYY') }}</div>
                        </td>
                        <td style="text-align: center;">
                            @if($item->dokumen)
                                ...
                                @else
                                    <a href="{{ route('surat-masuk.download', $item->id) }}" class="btn-download" title="Unduh Dokumen">
                        ...
                        <td>
                            <div class="action-buttons" style="justify-content: flex-end;">
                                @if(config('app.demo_mode', true))
                                    <button type="button" class="btn btn-secondary btn-sm" onclick="showPremiumContactPopup('Edit Surat Masuk')" title="Fitur Terkunci (Edisi GitHub)" style="padding: 8px 14px; font-weight: 700; border-radius: 10px; background: #f1f5f9; color: #94a3b8; border: 1px solid #e2e8f0;">
                                        <span>🔒 Edit</span>
                                    </button>
                                    <button type="button" class="btn btn-danger btn-sm" onclick="showPremiumContactPopup('Hapus Surat Masuk')" title="Fitur Terkunci (Edisi GitHub)" style="padding: 8px 14px; font-weight: 700; border-radius: 10px; background: #fff1f2; color: #f43f5e; border: 1px solid #ffe4e6;">
                                        <span>🔒 Hapus</span>
                                    </button>
                                @else
                                    <button type="button" class="btn btn-secondary btn-sm" onclick="openEditModal({{ $item->id }}, '{{ addslashes($item->no_surat) }}', '{{ addslashes($item->pengirim) }}', '{{ addslashes($item->perihal) }}', '{{ $item->kategori_id }}', '{{ $item->tgl_surat }}', '{{ $item->tgl_diterima }}', '{{ addslashes($item->keterangan ?: '') }}')" title="Edit Surat Masuk" style="padding: 8px 14px; font-weight: 700; border-radius: 10px;">
                                    ...
                                    <form id="delete-form-{{ $item->id }}" action="{{ route('surat-masuk.destroy', $item->id) }}" method="POST" style="margin:0;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete('delete-form-{{ $item->id }}', '{{ addslashes($item->no_surat) }}')" title="Hapus Surat Masuk" style="padding: 8px 14px; font-weight: 700; border-radius: 10px;">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.86弱a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                            <span>Hapus</span>
                                        </button>
                                    </form>
                                @endif
        ...
        <form action="{{ route('surat-masuk.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-grid-2">
                <div class="form-group" style="margin-bottom: 20px;">
                    ...
                    <input type="text" id="add-no_surat" name="no_surat" class="form-control" value="{{ old('no_surat') }}" placeholder="Contoh: 025/PT-EXT/2026/05" required autofocus style="width:100%; padding:12px 16px; background:#f8fafc; border:2px solid #e2e8f0; border-radius:14px; font-size:14px; font-weight:600;">
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                <label for="add-pengirim" class="form-label" style="display:block; font-size:12px; font-weight:700; margin-bottom:8px; text-transform:uppercase;">Pengirim / Instansi Asal *</label>
                <input type="text" id="add-pengirim" name="pengirim" class="form-control" value="{{ old('pengirim') }}" placeholder="Contoh: Kementerian BUMN / PT Telkom..." required style="width:100%; padding:12px 16px; background:#f8fafc; border:2px solid #e2e8f0; border-radius:14px; font-size:14px; font-weight:600;">
                @if($errors->any() && !old('_method'))
                    @error('pengirim') <span class="text-danger" style="font-size:12px; color:var(--danger); margin-top:6px; display:block;">{{ $message }}</span> @enderror
                @endif
            </div>
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                ...
                <input type="text" id="add-perihal" name="perihal" class="form-control" value="{{ old('perihal') }}" placeholder="Contoh: Undangan Rapat Koordinasi..." required style="width:100%; padding:12px 16px; background:#f8fafc; border:2px solid #e2e8f0; border-radius:14px; font-size:14px; font-weight:600;">
                @if($errors->any() && !old('_method'))
            ...
            <div class="form-grid-2">
                ...
                <div class="form-group" style="margin-bottom: 20px;">
                    <label for="add-tgl_diterima" class="form-label" style="display:block; font-size:12px; font-weight:700; margin-bottom:8px; text-transform:uppercase;">Tanggal Diterima *</label>
                    <input type="date" id="add-tgl_diterima" name="tgl_diterima" class="form-control" value="{{ old('tgl_diterima', date('Y-m-d')) }}" required style="width:100%; padding:12px 16px; background:#f8fafc; border:2px solid #e2e8f0; border-radius:14px; font-size:14px; font-weight:600;">
                    @if($errors->any() && !old('_method'))
                        @error('tgl_diterima') <span class="text-danger" style="font-size:12px; color:var(--danger); margin-top:6px; display:block;">{{ $message }}</span> @enderror
                    @endif
                </div>
            </div>
        ...
        <form id="edit-surat-form" action="#" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-grid-2">
                <div class="form-group" style="margin-bottom: 20px;">
                    ...
                    <input type="text" id="edit-no_surat" name="no_surat" class="form-control" value="{{ old('no_surat') }}" placeholder="Contoh: PTPN/SM/2026/05/001" required autofocus style="width:100%; padding:12px 16px; background:#f8fafc; border:2px solid #e2e8f0; border-radius:14px; font-size:14px; font-weight:600;">
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                <label for="edit-pengirim" class="form-label" style="display:block; font-size:12px; font-weight:700; margin-bottom:8px; text-transform:uppercase;">Pengirim / Instansi Asal *</label>
                <input type="text" id="edit-pengirim" name="pengirim" class="form-control" value="{{ old('pengirim') }}" placeholder="Contoh: Kementerian BUMN / PT Telkom..." required style="width:100%; padding:12px 16px; background:#f8fafc; border:2px solid #e2e8f0; border-radius:14px; font-size:14px; font-weight:600;">
                @if($errors->any() && old('_method') === 'PUT')
                    @error('pengirim') <span class="text-danger" style="font-size:12px; color:var(--danger); margin-top:6px; display:block;">{{ $message }}</span> @enderror
                @endif
            </div>
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                ...
                <input type="text" id="edit-perihal" name="perihal" class="form-control" value="{{ old('perihal') }}" placeholder="Contoh: Undangan Rapat Koordinasi..." required style="width:100%; padding:12px 16px; background:#f8fafc; border:2px solid #e2e8f0; border-radius:14px; font-size:14px; font-weight:600;">
                @if($errors->any() && old('_method') === 'PUT')
                    @error('perihal') <span class="text-danger" style="font-size:12px; color:var(--danger); margin-top:6px; display:block;">{{ $message }}</span> @enderror
                @endif
            </div>
            ...
            <div class="form-grid-2">
                ...
                <div class="form-group" style="margin-bottom: 20px;">
                    <label for="edit-tgl_diterima" class="form-label" style="display:block; font-size:12px; font-weight:700; margin-bottom:8px; text-transform:uppercase;">Tanggal Diterima *</label>
                    <input type="date" id="edit-tgl_diterima" name="tgl_diterima" class="form-control" value="{{ old('tgl_diterima') }}" required style="width:100%; padding:12px 16px; background:#f8fafc; border:2px solid #e2e8f0; border-radius:14px; font-size:14px; font-weight:600;">
                    @if($errors->any() && old('_method') === 'PUT')
                        @error('tgl_diterima') <span class="text-danger" style="font-size:12px; color:var(--danger); margin-top:6px; display:block;">{{ $message }}</span> @enderror
                    @endif
                </div>
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                <label for="edit-dokumen" class="form-label" style="display:block; font-size:12px; font-weight:700; margin-bottom:8px; text-transform:uppercase;">Ganti Dokumen Lampiran (PDF/JPG/PNG, Max {{ $maxUploadMB }}MB - Biarkan kosong jika tidak ingin mengganti)</label>
...
<script>
    ...
    $(document).ready(function() {
        $('#surat-masuk-table').DataTable({
            ...
            language: {
                search: "Cari Surat Masuk:",
                lengthMenu: "Tampilkan _MENU_ entri",
                info: "Menampilkan _START_ sampai _END_ dari _TOTAL_ surat masuk",
                infoEmpty: "Menampilkan 0 sampai 0 dari 0 surat masuk",
                infoFiltered: "(disaring dari _MAX_ total surat masuk)",
                zeroRecords: "Tidak ada data surat masuk yang cocok dengan pencarian",
                paginate: {
                    first: "Pertama",
                    last: "Terakhir",
                    next: "Selanjutnya",
    ...
    // Fungsi Buka & Tutup Modal Tambah Surat Masuk
    ...
    // Fungsi Buka & Tutup Modal Edit Surat Masuk
    function openEditModal(id, no_surat, pengirim, perihal, kategori_id, tgl_surat, tgl_diterima, keterangan) {
        ...
        if (modal && form) {
            ...
            document.getElementById('edit-pengirim').value = pengirim;
            ...
            document.getElementById('edit-tgl_diterima').value = tgl_diterima;
    ...
    @if($errors->any())
        document.addEventListener('DOMContentLoaded', function() {
            @if(old('_method') === 'PUT')
                openEditModal({{ old('id') ?? 0 }}, '{{ addslashes(old('no_surat')) }}', '{{ addslashes(old('pengirim')) }}', '{{ addslashes(old('perihal')) }}', '{{ old('kategori_id') }}', '{{ old('tgl_surat') }}', '{{ old('tgl_diterima') }}', '{{ addslashes(old('keterangan')) }}');
            @else
                openAddModal();
            @endif
        });
